...
${tutor.nombre || id}`)) return;
  ...
  // src/js/db/carImages.js
  var isPlaceholderImage = (url) => /picsum\.photos|cdn\.imagin\.studio/i.test(String(url || ""));
  var resolveCarImages = (car) => {
    const images = Array.isArray(car == null ? void 0 : car.images) ? car.images : [];
  ...
  var STORAGE_KEY = "choricar-ecommerce-store-v4";
  ...
  var inferBodyType = (car) => {
    if (car == null ? void 0 : car.bodyType) return car.bodyType;
    return BODY_TYPE_BY_MODEL[car == null ? void 0 : car.model] || "sedan";
  };
  var normalizeCar = (car) => ({
    ...car,
    bodyType: inferBodyType(car),
    color: (car == null ? void 0 : car.color) || "Gris",
    images: resolveCarImages(car),
    maintenance: Array.isArray(car == null ? void 0 : car.maintenance) ? car.maintenance : [],
    isPremium: Boolean(car == null ? void 0 : car.isPremium)
  });
  var normalizeUser = (user) => ({
    ...user,
    verified: Boolean(user == null ? void 0 : user.verified),
    favorites: Array.isArray(user == null ? void 0 : user.favorites) ? user.favorites : [],
    plan: (user == null ? void 0 : user.plan) === "premium" ? "premium" : "free"
  });
  var normalizeStoreShape = (parsed) => ({
  ...
  var readLegacyStore = () => {
    for (const key of LEGACY_STORAGE_KEYS) {
      const stored = readStore2(key);
      if (stored) return stored;
    }
    return null;
  };
  var overlaySeedImages = (store, seedCars = []) => {
    ...
    store.cars = store.cars.map((car) => {
      ...
      const stale = current.some(isPlaceholderImage) || current.length > 0 && current.every((src) => isPlaceholderImage(src));
      if (!stale) return car;
  ...
  var loadEcommerceStore = async ({
    ...
    const stored = readStore2() || readLegacyStore();
    if (stored) {
      memoryStore = {
        cars: ensureIds2(stored.cars, "car").map(normalizeCar),
        users: ensureIds2(stored.users, "user").map(normalizeUser),
    ...
    memoryStore = {
      cars: ensureIds2(Array.isArray(carsData.cars) ? carsData.cars : [], "car").map(
        normalizeCar
      ),
      users: ensureIds2(Array.isArray(usersData.users) ? usersData.users : [], "user").map(
        normalizeUser
      ),
  ...
  var getCarsBySellerId = (sellerId) => getStore().cars.filter((car) => car.sellerId === sellerId);
  var whatsappUrl = (phone, text = "") => {
  ...
  var createVehicleCardElement = (car, { favorited = false } = {}) => {
    ...
    const image = ((_a = car.images) == null ? void 0 : _a[0]) || "";
    const title = `${car.brand} ${car.model}`;
    const seller = getUserById(car.sellerId);
    const waHref = (seller == null ? void 0 : seller.phone) ? whatsappUrl(seller.phone, `Hola, me interesa el ${title} ${car.year} publicado en Choricar.`) : "";
    if (isPremium) {
    ...
    const planBadge = isPremium ? `<span class="vehicle-card__badge vehicle-card__badge--premium">\u2605 Destacado</span>` : `<span class="vehicle-card__badge vehicle-card__badge--free">Gratis</span>`;
    const whatsappControl = waHref ? `<a class="vehicle-card__whatsapp" href="${waHref}" target="_blank" rel="noopener noreferrer" aria-label="Contactar por WhatsApp">${WHATSAPP_ICON}<span>WhatsApp</span></a>` : "";
    article.innerHTML = `
  ...
  var isTruthyParam = (value) => value === "1" || value === "true" || value === true;
  var aliasKey = (value, map) => {
    if (!value) return "";
    const normalized = String(value).trim().toLowerCase().normalize("NFD").replace(/\p{M}/gu, "");
  ...
  var readFiltersFromUrl = () => {
    ...
    return {
      ...
      condition: aliasKey(params.get("condition") || params.get("condicion"), CONDITION_ALIASES) || params.get("condition") || "",
      transmission: params.get("transmission") || "",
      fuel: aliasKey(params.get("fuel") || params.get("combustible"), FUEL_ALIASES) || fuelFromTipo || params.get("fuel") || "",
      bodyType: aliasKey(params.get("bodyType") || params.get("carroceria"), BODY_ALIASES) || (!fuelFromTipo && bodyFromTipo ? bodyFromTipo : "") || params.get("bodyType") || "",
  ...
  var applyFilters = (cars, filters) => {
    ...
    if (isTruthyParam(filters.verifiedOnly)) {
      list = list.filter((c) => {
        var _a;
        return Boolean((_a = getUserById(c.sellerId)) == null ? void 0 : _a.verified);
      });
    }
  ...
  var applyFiltersToForm = (form, filters) => {
    if (!form) return;
    Object.entries(filters).forEach(([key, value]) => {
  ...
  var vehicleGrid = () => {
    ...
    const getFiltersFromForm = () => {
      ...
      return {
        ...
        hasPhotos: ((_a = form.querySelector('[name="hasPhotos"]')) == null ? void 0 : _a.checked) ? "1" : "",
        verifiedOnly: ((_b = form.querySelector('[name="verifiedOnly"]')) == null ? void 0 : _b.checked) ? "1" : "",
  ...
  var vehicleGallerySlider = () => {
    ...
    document.querySelectorAll(".vehicle-gallery-slider").forEach((root) => {
      ...
      const thumbsSlides = (thumbsEl == null ? void 0 : thumbsEl.querySelectorAll(".swiper-slide").length) || 0;
      ...
      root._swiper = new Swiper(el, {
        ...
        thumbs: root._thumbsSwiper ? { swiper: root._thumbsSwiper } : void 0
      });
  ...
  // src/js/modules/similarCarsSlider.js
  var destroySwiper2 = (instance) => {
    if (instance && typeof instance.destroy === "function") {
      instance.destroy(true, true);
    }
  };
  var similarCarsSlider = () => {
    if (typeof Swiper === "undefined") return;
  ...
  var similarCarsSlider_default = similarCarsSlider;
    ...
    document.querySelectorAll("[data-vehicle-detail]").forEach((root) => {
      ...
      const render = () => {
        ...
        const fillGallery = (selector) => {
          ...
          if (!gallery) return;
          gallery.innerHTML = "";
          const images = ((_a3 = car.images) == null ? void 0 : _a3.length) ? car.images : [];
          images.forEach((src) => {
        ...
        const waHref = (seller == null ? void 0 : seller.phone) ? whatsappUrl(seller.phone, inquiry) : "";
        ...
        if (sellerEl) {
          sellerEl.innerHTML = seller ? `
						...
						<div class="vehicle-detail__seller-copy">
							...
							<p class="vehicle-detail__seller-plan">Plan: ${seller.plan === "premium" ? "Premium" : "Gratis"}${seller.verified ? " \xB7 Verificado" : ""}</p>
        ...
        if (contact) {
          ...
          } else if (seller == null ? void 0 : seller.email) {
        ...
        if (similarList) {
          ...
          if (similar.length) {
            ...
            const favorites = (user2 == null ? void 0 : user2.favorites) || [];
  ...
  var optionLabel = (field, value) => {
    var _a;
    if (!value) return "\u2014";
    if ((field == null ? void 0 : field.name) === "condition") return conditionLabel(value);
    if ((field == null ? void 0 : field.name) === "fuel") return fuelLabel(value);
    if ((field == null ? void 0 : field.name) === "transmission") return transmissionLabel(value);
    if ((field == null ? void 0 : field.name) === "bodyType") return bodyTypeLabel(value);
    const match = (_a = field == null ? void 0 : field.options) == null ? void 0 : _a.find((opt) => String(opt.value) === String(value));
    return (match == null ? void 0 : match.label) || String(value);
  };
  var vehicleForm = () => {
    document.querySelectorAll("[data-vehicle-form]").forEach((root) => {
      ...
      const renderSummary = () => {
        ...
        const data = new FormData(form);
        const rows = (copy.fields || []).filter((field) => field.name !== "description").map((field) => {
        ...
        summaryEl.innerHTML = `
          ...
          (src, index) => `<img class="vehicle-form__summary-image" src="${src}" alt="Foto ${index + 1}" />`
        ).join("") : `<p>${((_a = copy.summary) == null ? void 0 : _a.emptyPhotos) || "Sin fotos"}</p>`}
				</div>
			`;
      };
      const validateStep = (step) => {
        if (step === 1) {
          ...
          const fields = (panel == null ? void 0 : panel.querySelectorAll("input, select, textarea")) || [];
        ...
        if (step === 2) {
          if (images.length < minFiles) {
            const message = `Adjunta al menos ${minFiles} fotos del veh\xEDculo`;
            setError(message);
      ...
      nextEl == null ? void 0 : nextEl.addEventListener("click", () => {
        if (!validateStep(currentStep)) return;
        showStep(Math.min(3, currentStep + 1));
      });
      prevEl == null ? void 0 : prevEl.addEventListener("click", () => {
